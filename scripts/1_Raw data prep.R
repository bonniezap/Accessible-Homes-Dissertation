...
###########################################
#Dependent variable
#DONE Falls - in last year, in last month

#Independent variable
#DONE Frailty based on Physical Frailty Phenotype

#Moderating variable
#DONE Press Index (accessibility of home)
#DONE individual accessible home features for sensitivity analysis
...
#Next tasks:
#Code sensitive data and demogs from r1 and r5 in a more elegant way. Remove merges on each round
#create a file that contains all the data from round 1 stable data as continuing in each round, then do the same for r5. 
#add to this the sensitive data. 
#make sure cohort 2011 demogs continue into rounds 5+. 
#Then do final merge to indep and dep vars

#covariate description from proposal:
#Covariates
#Sociodemographic, health, and support variables will be used as controls in models of frailty, home accessibility and falls. 
#1.Gender and race should be included as controls because men and women experience aging differently and because historically vulnerable populations are more likely to live in homes with barriers to AIP (Riffe et al., 2016; Rothstein, 2017). 
#2. Because lack of financial assets limits housing choices, housing type, wealth and income should be used as controls (Jenkins Morales & Robert, 2020). 
#3. Prior reports of dementia should be included because the literature clearly shows that people with experiencing dementia are more likely to have difficulty AIP (Chyr et al., 2020). 
#4. In order to isolate the effect of home accessibility, marital status and support in the home from others should also be included as controls (Vanleerberghe et al., 2019; Welti et al., 2020). 
#5. Vision impairment  and medication use that cause dizziness and sedation can contribute to falls in the home (Gray et al., 2019; Jamieson et al., 2019; La Grow et al., 2006; Letts et al., 2010).
#I will evaluate models with and without health controls as a form of sensitivity analysis to determine whether the proximal and distal variables that relate to the five measures of frailty (grip strength, gait speed, exhaustion, low activity, and unintentional weight loss) impact the interpretation of the results and the explanatory power of the models. This sensitivity analysis will also determine if these health controls are confounders that explain the relationship between frailty, falls and home accessibility. 
#6. These variables include self-reported health, number of health conditions, limitations with ADL, and use of assistive walking devices. 
#I will also evaluate the bivariate relationships between the covariates to determine if I need to engage in a formal test of multicollinearity.

#numbers below based on line of proposal description of covariates
#Time-invarying:
#DONE birthyr
#1. DONE Gender
#1. DONE Race
...
#DONE age
#2. housing type - check missing values in subsequent rounds. Is this due to not new home? ROUND 2 it is not. Continue to check@!

#2. NOT USING BC ONLY SMALL SUBSET ANSWERED - wealth
#2. NOT USING BC ONLY SMALL SUBSET ANSWERED - Income
#3. Prior reports of dementia
#4. marital status
#4. Support in the home
#5. Vision impairment
#5. Medication use that causes dizziness and sedation
#6. self-reported health
#6. number of health conditions
#6. ADL limitations
#6. Use of assistive walking devices

#locate time-varying variables of interest in the sensitive data files that is not in the regular data files
#make file that is spid, round, and the vars
#merge by spid + round
#Or go back and do it earlier in the program when can merge only by spid, individually by round

#4/10/23
#1. Look at the data across all waves and get a sense of it. COMPLETE
#2. Check final directionality of frailty and acc COMPLETE
...
#6. Look at variables in equation and transform for normality. Mean center as needed (does r do this automatically in the analytic step?) Keep original variables for demographics. 

#2/4/23 Crosswalking task steps: (COMPLETE)
...
#2/4/23 Get official permission from NHATS to use birth year and other sensitive data in dissertation. Ask Jeff on Tuesday about it - I CAN USE IT BASED ON MY USE AGREEMENT
...
round1 <- 
  ...
  select(spid,r1dbirthyr, r1dintvwrage) %>% mutate(birthyr=r1dbirthyr, age=r1dintvwrage),
(read_sas("~/Documents/NHATS/NHATS Public Data/Public Round 1/NHATS_R1_Final_Release_SAS_V3/NHATS_Round_1_SP_File.sas7bdat", NULL)) %>% 
    select(spid
           ,el1higstschl
           #,ia1skbdjtwrt #total wealth
           #,ia1toincesjt #joint total income
           ,fl1hotype #own, rent or other (home type)
           ,hh1martlstat
           ,mo1insdhlp
...
vars = c('spid', 'female', 'count', 'round', 'birthyr','age', 'educ', 'whitenothisp', 'blacknothisp', 'hispanic', 'otherrace', 'falls2011', 'competence2011', 'frailty2011', 'pfp2011', 'press2011', 'accburden2011', 'recentstructmod2011', 'bldgextst2011', 'inbldgst2011', 'inhomest2011', 'hc1worryfall'
         ,'hp1ownrentot'
         ,'dm1helpmobil'
         ,'mo1insdcane'
         ,'mo1insdwalkr'
...
r1same<-r1 %>% rename(
  ...
  recentstructmod=recentstructmod2011,
  bldgextst=bldgextst2011,
  inbldgst=inbldgst2011,
  inhomest=inhomest2011,
  ownrent=hp1ownrentot,
  mobhelp=dm1helpmobil,
  cane=mo1insdcane,
  walker=mo1insdwalkr,
  balance=ss1prbbalcrd,
  worryfall=hc1worryfall,
  painlimits=ss1painlimts,
  breathinglimits=ss1prbbrlimt,
  upperstrlimits=ss1uplimtact,
  lowerstrimpacts=ss1lwrbodimp) %>% 
...
round2 <- 
  merge ((r1 %>% select(spid, female)),read_sas("~/Documents/NHATS/NHATS Public Data/Public Round 2/NHATS_R2_Final_Release_SAS_V3/NHATS_Round_2_SP_File_v2.sas7bdat", NULL)) %>%  
  select(spid
         ,female
         ,fl2hotype #own, rent or other (home type)
         ,hp2ownrentot
         ,re2dresistrct 
         ,re2newstrct
         ,hh2martlstat
...
#Create tidy dataset
vars = c('spid', 'count', 'round', 'female', 'falls2012', 'competence2012', 'frailty2012', 'pfp2012', 'press2012', 'accburden2012','recentstructmod2012', 'bldgextst2012', 'inbldgst2012', 'inhomest2012', 'hc2worryfall'
         ,'hp2ownrentot'
         ,'dm2helpmobil'
         ,'mo2insdcane'
         ,'mo2insdwalkr'
...
r2same<-r2 %>% rename(
  ...
  recentstructmod=recentstructmod2012,
  bldgextst=bldgextst2012,
  inbldgst=inbldgst2012,
  inhomest=inhomest2012,
  ownrent=hp2ownrentot,
  mobhelp=dm2helpmobil,
  cane=mo2insdcane,
  walker=mo2insdwalkr,
  balance=ss2prbbalcrd,
  worryfall=hc2worryfall,
  painlimits=ss2painlimts,
  breathinglimits=ss2prbbrlimt,
  upperstrlimits=ss2uplimtact,
  lowerstrimpacts=ss2lwrbodimp
    ) %>% 
  select(spid
         ,count
         ,round
...
rounds12<-merge ((r1 %>% select(spid, female, birthyr, educ, whitenothisp, blacknothisp, hispanic, otherrace)),bind_rows(r1same, r2same))
...
round3 <- merge ((r1 %>% select(spid, female)),read_sas("~/Documents/NHATS/NHATS Public Data/Public Round 3/NHATS_R3_Final_Release_SAS_V2/NHATS_Round_3_SP_File.sas7bdat", NULL)) %>%  
  select(spid
#         ,ia3skbdjtwrt #total wealth
         ,ia3toincesjt #joint total income
         ,fl3hotype #own, rent or other (home type)
         ,hp3ownrentot
         ,hh3martlstat
         ,mo3insdhlp
         ,dm3helpmobil
...
#Create tidy dataset for Tableau heat map, of falls plotted over frailty and acc
vars = c('spid','count', 'round', 'falls2013', 'competence2013', 'frailty2013', 'pfp2013', 'press2013', 'accburden2013','recentstructmod2013', 'hc3worryfall', 'bldgextst2013', 'inbldgst2013', 'inhomest2013'
         ,'hp3ownrentot'
         ,'dm3helpmobil'
         ,'mo3insdcane'
         ,'mo3insdwalkr'
...
r3same<-r3 %>% rename(
  ...
  recentstructmod=recentstructmod2013,
  bldgextst=bldgextst2013,
  inbldgst=inbldgst2013,
  inhomest=inhomest2013,
  ownrent=hp3ownrentot,
  mobhelp=dm3helpmobil,
  cane=mo3insdcane,
  walker=mo3insdwalkr,
  balance=ss3prbbalcrd,
  worryfall=hc3worryfall,
  painlimits=ss3painlimts,
  breathinglimits=ss3prbbrlimt,
  upperstrlimits=ss3uplimtact,
  lowerstrimpacts=ss3lwrbodimp
  ) %>% 
  select(spid
         ,count
         ,round
...
rounds123<-merge ((r1 %>% select(spid, female, birthyr, educ, whitenothisp, blacknothisp, hispanic, otherrace)),bind_rows(r1same, r2same, r3same))
...
tabled(round4$hp4ownrentot)
#Create tidy dataset for Tableau heat map, of falls plotted over frailty and acc
vars = c('spid','count', 'round', 'falls2014', 'competence2014', 'frailty2014', 'pfp2014', 'press2014', 'accburden2014','recentstructmod2014', 'bldgextst2014', 'inbldgst2014', 'inhomest2014', 'hc4worryfall'
         ,'hp4ownrentot'
         ,'dm4helpmobil'
         ,'mo4insdcane'
         ,'mo4insdwalkr'
...
r4same<-r4 %>% rename(
  ...
  recentstructmod=recentstructmod2014,
  bldgextst=bldgextst2014,
  inbldgst=inbldgst2014,
  inhomest=inhomest2014,
  ownrent=hp4ownrentot,
  mobhelp=dm4helpmobil,
  cane=mo4insdcane,
  walker=mo4insdwalkr,
  balance=ss4prbbalcrd,
  worryfall=hc4worryfall,
  painlimits=ss4painlimts,
  breathinglimits=ss4prbbrlimt,
  upperstrlimits=ss4uplimtact,
  lowerstrimpacts=ss4lwrbodimp) %>% 
  select(spid
         ,count
         ,round
         ,falls
...
#Stack repeat measures with round name and then add on demographics. 
rounds1234<-merge ((r1 %>% select(spid, female, birthyr, educ, whitenothisp, blacknothisp, hispanic, otherrace)),bind_rows(r1same, r2same, r3same, r4same))
...
#################################################################################
#round5foo <- 
#  merge (read_sas("~/Documents/NHATS/NHATS Sensitive Data/Sensitive Round 5/NHATS_Round_5_SP_Sen_Dem_SAS/NHATS_Round_5_SP_Sen_Dem_File.sas7bdat", NULL) %>% 
#           select(spid,r5dbirthyr, r5dintvwrage) %>% mutate(birthyr=r5dbirthyr, age=r5dintvwrage),
#         (read_sas("~/Documents/NHATS/NHATS Public Data/Public Round 5/NHATS_R5_Final_Release_SAS_V3/NHATS_Round_5_SP_File_v2.sas7bdat", NULL)) %>% 
round5 <- merge ((r1 %>% select(spid, female)),read_sas("~/Documents/NHATS/NHATS Public Data/Public Round 5/NHATS_R5_Final_Release_SAS_V3/NHATS_Round_5_SP_File_v2.sas7bdat", NULL)) %>%  
  select(spid
         ...
         ,el5higstschl
         ,hp5ownrentot
         ,fl5hotype #own, rent or other (home type)
         ,hh5martlstat
         ,mo5insdhlp
         ,dm5helpmobil
         ,mo5insdcane
  ...
  mutate(
    count=1,
    round='2015',
      #prepare permanent demogs from round 5, gender and race/eth
      female=case_when( #recodes 0 to male and 1 to female
        r5dgender==1~0,
        r5dgender==2~1,
        TRUE~NA_real_),
      whitenothisp=case_when(  #recodes race to whitenonH or not
        rl5dracehisp==1~1, #whitenonH
        (rl5dracehisp==2 |rl5dracehisp==3 |rl5dracehisp==4 | rl5dracehisp==5)~0, #not White Non Hispanic
        #Race 1=WNH 2=BNH 3=OtherNH 4=Hispanic 5=multiracial 6=DK/RF
        TRUE~NA_real_),
      blacknothisp=case_when(  #recodes race to blacknonH or not
        rl5dracehisp==2~1, #blacknonH
        (rl5dracehisp==1 |rl5dracehisp==3 |rl5dracehisp==4 | rl5dracehisp==5)~0, #not Black Non Hispanic
        #Race 1=WNH 2=BNH 3=OtherNH 4=Hispanic 5=multiracial 6=DK/RF
        TRUE~NA_real_),
      hispanic=case_when(  #recodes race to Hispanic or not
        rl5dracehisp==4~1, #Hispanic
        (rl5dracehisp==1 |rl5dracehisp==2 |rl5dracehisp==3 | rl5dracehisp==5)~0, #not Hispanic
        #Race 1=WNH 2=BNH 3=OtherNH 4=Hispanic 5=multiracial 6=DK/RF
        TRUE~NA_real_),
      otherrace=case_when(  #recodes race to other/multi
        (rl5dracehisp==3 | rl5dracehisp==5)~1, #other/multi
        (rl5dracehisp==1 | rl5dracehisp==2 |rl5dracehisp==4)~0, #not other/multi
        #Race 1=WNH 2=BNH 3=OtherNH 4=Hispanic 5=multiracial 6=DK/RF
        TRUE~NA_real_),
    #This next section preps variables needed for one or more of the 5 frailty metrics  
...
#Create tidy dataset for Tableau heat map, of falls plotted over frailty and acc
vars = c('spid','count', 'round', 'female', 'whitenothisp', 'blacknothisp', 'hispanic', 'otherrace', 'falls2015', 'competence2015', 'frailty2015', 'pfp2015', 'press2015', 'accburden2015','recentstructmod2015', 'bldgextst2015', 'inbldgst2015', 'inhomest2015', 'hc5worryfall'
         ,'hp5ownrentot'
         ,'dm5helpmobil'
         ,'mo5insdcane'
         ,'mo5insdwalkr'
...
r5same<-r5 %>% rename(
  ...
  bldgextst=bldgextst2015,
  inbldgst=inbldgst2015,
  inhomest=inhomest2015,
  ownrent=hp5ownrentot,
  mobhelp=dm5helpmobil,
  cane=mo5insdcane,
  walker=mo5insdwalkr,
  balance=ss5prbbalcrd,
  worryfall=hc5worryfall,
  painlimits=ss5painlimts,
  breathinglimits=ss5prbbrlimt,
  upperstrlimits=ss5uplimtact,
  lowerstrimpacts=ss5lwrbodimp
) %>% 
  select(spid
         ,count
         ,round
...
#Stack repeat measures with round name and then add on demographics. 
rounds12345<-merge ((r1 %>% select(spid, female, birthyr, educ, whitenothisp, blacknothisp, hispanic, otherrace)),bind_rows(r1same, r2same, r3same, r4same, r5same))
...
#Create tidy dataset for Tableau heat map, of falls plotted over frailty and acc
vars = c('spid','count', 'round', 'falls2016', 'competence2016', 'frailty2016', 'pfp2016', 'press2016', 'accburden2016','recentstructmod2016', 'bldgextst2016', 'inbldgst2016', 'inhomest2016', 'hc6worryfall'
         ,'hp6ownrentot'
         ,'dm6helpmobil'
         ,'mo6insdcane'
         ,'mo6insdwalkr'
...
r6same<-r6 %>% rename(
  ...
  bldgextst=bldgextst2016,
  inbldgst=inbldgst2016,
  inhomest=inhomest2016,
  ownrent=hp6ownrentot,
  mobhelp=dm6helpmobil,
  cane=mo6insdcane,
  walker=mo6insdwalkr,
  balance=ss6prbbalcrd,
  worryfall=hc6worryfall,
  painlimits=ss6painlimts,
  breathinglimits=ss6prbbrlimt,
  upperstrlimits=ss6uplimtact,
  lowerstrimpacts=ss6lwrbodimp
) %>% 
  select(spid
         ,count
         ,round
...
#Stack repeat measures with round name and then add on demographics. 
rounds123456<-merge ((r1 %>% select(spid, female, birthyr, educ, whitenothisp, blacknothisp, hispanic, otherrace)),bind_rows(r1same, r2same, r3same, r4same, r5same, r6same))
...
#Create tidy dataset for Tableau heat map, of falls plotted over frailty and acc
vars = c('spid','count', 'round', 'falls2017', 'competence2017', 'frailty2017', 'pfp2017', 'press2017', 'accburden2017','recentstructmod2017', 'bldgextst2017', 'inbldgst2017', 'inhomest2017', 'hc7worryfall'
         ,'hp7ownrentot'
         ,'dm7helpmobil'
         ,'mo7insdcane'
         ,'mo7insdwalkr'
...
r7same<-r7 %>% rename(
  ...
  bldgextst=bldgextst2017,
  inbldgst=inbldgst2017,
  inhomest=inhomest2017,
  ownrent=hp7ownrentot,
  mobhelp=dm7helpmobil,
  cane=mo7insdcane,
  walker=mo7insdwalkr,
  balance=ss7prbbalcrd,
  worryfall=hc7worryfall,
  painlimits=ss7painlimts,
  breathinglimits=ss7prbbrlimt,
  upperstrlimits=ss7uplimtact,
  lowerstrimpacts=ss7lwrbodimp
) %>% 
  select(spid
         ,count
         ,round
...
#Stack repeat measures with round name and then add on demographics. 
rounds1234567<-merge ((r1 %>% select(spid, female, birthyr, educ, whitenothisp, blacknothisp, hispanic, otherrace)),bind_rows(r1same, r2same, r3same, r4same, r5same, r6same, r7same))
...
#Create tidy dataset for Tableau heat map, of falls plotted over frailty and acc
vars = c('spid','count', 'round', 'falls2018', 'competence2018', 'frailty2018', 'pfp2018', 'press2018', 'accburden2018', 'recentstructmod2018', 'bldgextst2018', 'inbldgst2018', 'inhomest2018', 'hc8worryfall'
         ,'hp8ownrentot'
         ,'dm8helpmobil'
         ,'mo8insdcane'
         ,'mo8insdwalkr'
...
r8same<-r8 %>% rename(
  ...
  bldgextst=bldgextst2018,
  inbldgst=inbldgst2018,
  inhomest=inhomest2018,
  ownrent=hp8ownrentot,
  mobhelp=dm8helpmobil,
  cane=mo8insdcane,
  walker=mo8insdwalkr,
  balance=ss8prbbalcrd,
  worryfall=hc8worryfall,
  painlimits=ss8painlimts,
  breathinglimits=ss8prbbrlimt,
  upperstrlimits=ss8uplimtact,
  lowerstrimpacts=ss8lwrbodimp
) %>% 
  select(spid
         ,count
         ,round
...
#Stack repeat measures with round name and then add on demographics. 
rounds12345678<-merge ((r1 %>% select(spid, female, birthyr, educ, whitenothisp, blacknothisp, hispanic, otherrace)),bind_rows(r1same, r2same, r3same, r4same, r5same, r6same, r7same, r8same))
...
#Create tidy dataset for Tableau heat map, of falls plotted over frailty and acc
vars = c('spid','count', 'round', 'falls2019', 'competence2019', 'frailty2019', 'pfp2019', 'press2019', 'accburden2019','recentstructmod2019', 'bldgextst2019', 'inbldgst2019', 'inhomest2019', 'hc9worryfall'
         ,'hp9ownrentot'
         ,'dm9helpmobil'
         ,'mo9insdcane'
         ,'mo9insdwalkr'
...
r9same<-r9 %>% rename(
  ...
  bldgextst=bldgextst2019,
  inbldgst=inbldgst2019,
  inhomest=inhomest2019,
  ownrent=hp9ownrentot,
  mobhelp=dm9helpmobil,
  cane=mo9insdcane,
  walker=mo9insdwalkr,
  balance=ss9prbbalcrd,
  worryfall=hc9worryfall,
  painlimits=ss9painlimts,
  breathinglimits=ss9prbbrlimt,
  upperstrlimits=ss9uplimtact,
  lowerstrimpacts=ss9lwrbodimp
) %>% 
  select(spid
         ,count
         ,round
...
#Stack repeat measures with round name and then add on demographics. 
rounds123456789<-merge ((r1 %>% select(spid, female, birthyr, educ, whitenothisp, blacknothisp, hispanic, otherrace)),bind_rows(r1same, r2same, r3same, r4same, r5same, r6same, r7same, r8same, r9same))
#write.csv (rounds123456789,"~/Google Drive/000Albright Dissertation Progress/Dissertation Analytic Work/rounds123456789.csv")




##Round 10 may be used for outcomes only, but for now don't use it because it doesn't have full FRAILTY components to make PFP due to COVID no in-person interviews
#################################################################################
####################ROUND 10######################################################
#################################################################################
...
#Create tidy dataset for Tableau heat map, of falls plotted over frailty and acc
vars = c('spid','count', 'round', 'falls2020',  'press2020', 'accburden2020','bldgextst2020', 'inbldgst2020', 'inhomest2020','recentstructmod2020', 'hc10worryfall')
r10<-round10[vars]
...
r10same<-r10 %>% rename(
  falls=falls2020,
  bldgextst=bldgextst2020,
  inbldgst=inbldgst2020,
  inhomest=inhomest2020,
  worryfall=hc10worryfall,
  press=press2020,
...
#Stack repeat measures with round name and then add on demographics. 
rounds12345678910<-merge ((r1 %>% select(spid, female, birthyr, educ, whitenothisp, blacknothisp, hispanic, otherrace)),bind_rows(r1same, r2same, r3same, r4same, r5same, r6same, r7same, r8same, r9same, r10same))
#write.csv (rounds12345678910,"clean_data/allcleanrounds.csv")


#check demogs by round
#tabled(rounds123456789$birthyr, rounds123456789$round)
#tabled(rounds123456789$whitenothisp, rounds123456789$round)



#Demogs, time varying, from sensitive data files:
sens1 <- 
  read_sas("~/Documents/NHATS/NHATS Sensitive Data/Sensitive Round 1/NHATS_Round_1_SP_Sen_Dem_Files_SAS/NHATS_Round_1_SP_Sen_Dem_File.sas7bdat", NULL) %>% 
           select(spid,r1dintvwrage, r1dbirthyr) %>% mutate(age=r1dintvwrage, round='2011', deathage=0, birthyr=r1dbirthyr)

sens2 <- 
  read_sas("~/Documents/NHATS/NHATS Sensitive Data/Sensitive Round 2/NHATS_Round_2_SP_Sen_Dem_SAS/NHATS_Round_2_SP_Sen_Dem_File.sas7bdat", NULL) %>% 
  select(spid,r2dintvwrage, r2ddeathage) %>% mutate(age=r2dintvwrage, deathage=case_when(r2ddeathage>1~r2ddeathage,TRUE~0), round='2012') 

sens3 <- 
  read_sas("~/Documents/NHATS/NHATS Sensitive Data/Sensitive Round 3/NHATS_Round_3_SP_Sen_Dem_SAS/NHATS_Round_3_SP_Sen_Dem_File.sas7bdat", NULL) %>% 
  select(spid,r3dintvwrage, r3ddeathage) %>% mutate(age=r3dintvwrage, deathage=case_when(r3ddeathage>1~r3ddeathage,TRUE~0), round='2013') 

sens4 <- 
  read_sas("~/Documents/NHATS/NHATS Sensitive Data/Sensitive Round 4/NHATS_Round_4_SP_Sen_Dem_SAS/NHATS_Round_4_SP_Sen_Dem_File.sas7bdat", NULL) %>% 
  select(spid,r4dintvwrage, r4ddeathage) %>% mutate(age=r4dintvwrage, deathage=case_when(r4ddeathage>1~r4ddeathage,TRUE~0), round='2014') 

sens5 <- 
  read_sas("~/Documents/NHATS/NHATS Sensitive Data/Sensitive Round 5/NHATS_Round_5_SP_Sen_Dem_SAS/NHATS_Round_5_SP_Sen_Dem_File.sas7bdat", NULL) %>% 
  select(spid,r5dintvwrage, r5ddeathage, r5dbirthyr) %>% mutate(age=r5dintvwrage, deathage=case_when(r5ddeathage>1~r5ddeathage,TRUE~0), round='2015') 

sens6 <- 
  read_sas("~/Documents/NHATS/NHATS Sensitive Data/Sensitive Round 6/NHATS_R6_SP_Sen_Dem_SAS/NHATS_Round_6_SP_Sen_Dem_File.sas7bdat", NULL) %>% 
  select(spid,r6dintvwrage, r6ddeathage) %>% mutate(age=r6dintvwrage, deathage=case_when(r6ddeathage>1~r6ddeathage,TRUE~0), round='2016') 

sens7 <- 
  read_sas("~/Documents/NHATS/NHATS Sensitive Data/Sensitive Round 7/NHATS_R7_SP_Sen_Dem_SAS/NHATS_Round_7_SP_Sen_Dem_File.sas7bdat", NULL) %>% 
  select(spid,r7dintvwrage, r7ddeathage) %>% mutate(age=r7dintvwrage, deathage=case_when(r7ddeathage>1~r7ddeathage,TRUE~0), round='2017') 

sens8 <- 
  read_sas("~/Documents/NHATS/NHATS Sensitive Data/Sensitive Round 8/NHATS_R8_SP_Sen_Dem_SAS/NHATS_Round_8_SP_Sen_Dem_File.sas7bdat", NULL) %>% 
  select(spid,r8dintvwrage, r8ddeathage) %>% mutate(age=r8dintvwrage, deathage=case_when(r8ddeathage>1~r8ddeathage,TRUE~0), round='2018') 

sens9 <- 
  read_sas("~/Documents/NHATS/NHATS Sensitive Data/Sensitive Round 9/NHATS_R9_SP_Sen_Dem_SAS/NHATS_Round_9_SP_Sen_Dem_File.sas7bdat", NULL) %>% 
  select(spid,r9dintvwrage, r9ddeathage) %>% mutate(age=r9dintvwrage, deathage=case_when(r9ddeathage>1~r9ddeathage,TRUE~0), round='2019') 

sens10 <- 
  read_sas("~/Documents/NHATS/NHATS Sensitive Data/Sensitive Round 10/NHATS_R10_SP_Sen_Dem_SAS/NHATS_Round_10_SP_Sen_Dem_File.sas7bdat", NULL) %>% 
  select(spid,r10dintvwrage, r10ddeathage) %>% mutate(age=r10dintvwrage, deathage=case_when(r10ddeathage>1~r10ddeathage,TRUE~0), round='2020') 

sensallall<-bind_rows(sens1, sens2, sens3, sens4, sens5, sens6, sens7, sens8, sens9, sens10) %>% na_if (-1) %>% na_if(-7) %>% na_if(-8)
vars = c('spid', 'round', 'age', 'deathage')
sensall<-sensallall[vars]

tabled (sensall$deathage, sensall$round)

a<-merge (x=sensall, y=rounds12345678910, by=c("spid", "round"))

write.csv (a,"clean_data/allrounds.csv")
